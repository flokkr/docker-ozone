...
*** Test Cases ***
RpcClient with port
   Test ozone shell       o3://            ozone-om-0.ozone-om:9862     ${prefix}-rpcwoport

RpcClient volume acls
   Test Volume Acls       o3://            ozone-om-0.ozone-om:9862     ${prefix}-rpcwoport2

RpcClient bucket acls
    Test Bucket Acls      o3://            ozone-om-0.ozone-om:9862     ${prefix}-rpcwoport2

RpcClient key acls
    Test Key Acls         o3://            ozone-om-0.ozone-om:9862     ${prefix}-rpcwoport2
...
Test key handling
    [arguments]     ${protocol}         ${server}       ${volume}
                    Execute             ozone sh key put ${protocol}${server}/${volume}/bb1/key1 /opt/ozone/NOTICE.txt
                    Execute             rm -f /tmp/NOTICE.txt.1
                    Execute             ozone sh key get ${protocol}${server}/${volume}/bb1/key1 /tmp/NOTICE.txt.1
                    Execute             diff -q /opt/ozone/NOTICE.txt /tmp/NOTICE.txt.1
...
Test key Acls
    [arguments]     ${protocol}         ${server}       ${volume}
    Execute         ozone sh key put ${protocol}${server}/${volume}/bb1/key2 /opt/ozone/NOTICE.txt
